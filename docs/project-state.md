...
- `npm run tauri build` succeeded at HEAD `9035549` plus the current five small working-tree fixes. It produced the x64 MSI and NSIS bundles. MSI administrative extraction (`msiexec /a`) returned 0, and the executable extracted from that MSI launched as a production `tauri.localhost` app with all five WebViews. Ordinary install, upgrade, and uninstall were not run against the existing user installation.
- Actual packaged QA at 200% scale on one 2880x1800 physical monitor (work area 2880x1704) showed Main 855x760, PIP Compact 312x116 and Expanded 312x194, Pet 56x56, Calendar 311x433, and Quick Add 810x126 in logical pixels. Main and the auxiliary windows fit the observed work area. Main/PIP/Pet/Calendar/Quick Add screenshots had no release-blocking clipping. PIP and Quick Add reported always-on-top; PIP Acrylic and Pet/Calendar/Quick Add transparency rendered in the packaged WebViews.
- Main X and native close minimized Main to the taskbar rather than making it irretrievable. Quick Add native close and Escape hid the reusable window. The Main settings switches showed PIP and Calendar; PIP Expanded and Pet transitions worked, and Enter on the focused Pet restored Compact. A Calendar moved to physical `(10000,10000)` recovered inside the monitor work area on reopen. The five-window process remained intact.
- The packaged WebView accessibility trees exposed 20 Main, 8 PIP, 1 Pet, 33 Calendar, and 5 Quick Add interactive controls; none of these lacked an accessible name or focusable property. This is a blocker-level check, not a full accessibility audit. The global `Ctrl+Shift+Space` input path was not actually fired in packaged QA because the available Windows Computer Use surface returned no apps; Rust registration and the in-app Quick Add opening path are present, but the global trigger remains unverified.
- Visibility preferences were restored to their pre-QA values (`pip=false`, `calendar=false`, stored PIP mode `expanded`). Relaunch restored Main visible and the auxiliaries hidden with those same preferences. The Credential Manager target's presence was unchanged; no secret value was read or printed. One paused FocusSession remained paused. The SQLite file hash changed across a restart, while the latest domain `updated_at` values predated the second launch; this pass did not capture a pre-run logical row fingerprint, so full row-for-row persistence equivalence is **unverified**.
- The repository and bundle configuration contain no product decision for what uninstall should do to SQLite, WebView settings, or the Credential Manager secret. The generated NSIS script has an optional **Delete app data** checkbox that removes the roaming/local bundle directories when selected, but does not mention the Credential Manager target. The generated MSI removes install files without an explicit app-data/credential rule. Do not uninstall against a real user profile until the product policy, installer behavior, and safe test environment are settled.
- Multiple-monitor attachment/removal and a small effective work area (for example, 1920x1080 at 150% scale) were not available for actual QA. Main's 855x760 logical minimum and Quick Add's 810 logical width still need a targeted package check on such displays. The fixed-size/overflow layout is a plausible clipping risk, not a reproduced failure on the tested monitor.
- The M03 window fixes cover shortcut-registration failure tolerance, recoverable Main close, Quick Add native close, Pet keyboard activation, and Calendar work-area recovery. The replacement package was tested, but the pre-fix HEAD behavior was not separately packaged and measured. No domain, migration, or Figma source changed.
- After the last source change, `npm run build`, every existing `npm run test:*` script, `cargo test` (19 passing tests), `cargo check`, `cargo fmt --check`, and `cargo clippy --all-targets -- -D warnings` passed. `git diff --check` passed. Both bundles report `NotSigned`, as expected for this scope.
- Temporary QA scripts were removed from the repository. The local temp MSI extraction, test logs, and five QA screenshots remain outside Git because automatic command review rejected their deletion; remove only those `study-os-m03-*` temp paths when deletion is permitted. The ignored release bundles remain under `src-tauri/target/` for review and are not tracked.
- Remaining M03 gates before a release/install claim: decide the uninstall data/credential policy, exercise safe installer upgrade/uninstall behavior in an isolated profile, verify the actual global shortcut, verify a small work area and multiple-monitor recovery, and capture a before/after logical persistence fingerprint. These are verification/product-decision blockers, not evidence that the observed 200% single-monitor package failed.
