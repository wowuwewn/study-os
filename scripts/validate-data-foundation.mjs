...
const migration = await readFile(resolve(root, "src-tauri/migrations/001_initial.sql"), "utf8");
...
const actualTables = db
  ...
  .all()
  .map(({ name }) => name);
assert.deepEqual(actualTables, expectedTables, "migration 001 should create the complete schema");
...
db.close();
console.log("data foundation validation passed: migration, idempotent seed, task CRUD, focus lifecycle, restart recovery");
